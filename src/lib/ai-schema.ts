...
export const TASK_PROMPTS: Record<string, string> = {
  "label-transitions": `You are a viral video editing expert. Given cut-detection data (timestamps, frame-difference deltas, brightness), label each cut with the most likely transition type. You MUST pick "type" only from this exact list: ${TRANSITIONS.map((t) => t.type).join(", ")}. Respond with ONLY a JSON object, no prose, no markdown fences: {"transitions":[{"time":number,"type":string,"description":string}]}`,
